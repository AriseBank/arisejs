...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = aco.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = aco.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = aco.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.aco);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = aco.crypto.fromBytes(aco.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(aco.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = aco.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = aco.crypto.verifySecondSignature(vt, aco.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
